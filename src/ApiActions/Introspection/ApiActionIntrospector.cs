...
namespace ApiActions.Introspection
{
    ...
    public class ApiActionIntrospector : IApiActionSummaryFactory, IApiActionDescriptionFactory,
        IApiActionResponseInfoFactory, IApiActionRequestMethodsFactory, IApiActionRequestTypeFactory,
        IApiActionCategoryFactory, IApiActionDeprecationFactory
    {
 ...
 GetAttribute<System.ComponentModel.DescriptionAttribute>(apiActionType)?.Description;
            ...
            return GetAttributes<IHasDescription>(apiActionType).Select(a => a.Description)
        ...
        public virtual IApiActionResponseInfo[] CreateResponses(Type apiActionType)
        {
            ...
            var classLevelResponseTypes =
                GetAttributes<IHasApiActionResponseInfo>(apiActionType).SelectMany(a => a.Responses);
            // ReSharper disable once SuspiciousTypeConversion.Global
            var methodLevelResponseTypes = apiActionType.GetMethods().SelectMany(mi => mi.GetCustomAttributes())
                .OfType<IHasApiActionResponseInfo>().SelectMany(a => a.Responses);
        ...
        {
            if (apiActionType == null) throw new ArgumentNullException(nameof(apiActionType));
            return GetAttributes<IHasRequestMethods>(apiActionType).SelectMany(a => a.RequestMethods)
                .Select(m => m.ToUpperInvariant()).Distinct().OrderBy(m => m).ToArray();
        }
        ...
        public virtual bool CreateIsDeprecated(Type apiActionType)
        {
            ...
            return GetAttributes<IVersionEdgeFactory>(apiActionType).SelectMany(f => f.GetVersionEdges()).Distinct()
                       .Count() > 1;
        }
