...
using ApiActions.Conversion;
using ApiActions.Execution;
using ApiActions.Initialization;
using ApiActions.Introspection;
using ApiActions.Responses.Construction;
...
namespace Microsoft.Extensions.DependencyInjection
{
    public static class ApiActionsServiceCollectionExtensions
    {
        ...
        public static IServiceCollection AddApiActions(this IServiceCollection services, Assembly assembly,
            string parentNamespace = null, string routePrefix = null)
        {
            if (services == null) throw new ArgumentNullException(nameof(services));
            ...
            var types = assembly.GetTypes()
                .Where(t =>
                {
                    ...
                    if (ti.Namespace == null ||
                        !ti.Namespace.StartsWith(preparedNamespace, StringComparison.OrdinalIgnoreCase) &&
                        !ti.Namespace.Equals(preparedParentNamespace))
                    {
                        return false;
                    }
            ...
            foreach (var type in types)
            {
                ...
                var namespaceSection =
                    typeInfo.FullName.Substring(0, typeInfo.FullName.Length - typeInfo.Name.Length - 1);

                namespaceSection = namespaceSection.Length > preparedNamespace.Length
                    ? namespaceSection.Substring(preparedNamespace.Length).Replace('.', '/')
                    : string.Empty;
                ...
                foreach (var routeConstraint in typeInfo.GetCustomAttributes().Where(a => a is IRouteConstraint)
                    .Cast<IRouteConstraint>())
                {
                    ...
                    if (constraintDictionary.ContainsKey(key))
                    {
                        throw new InvalidOperationException(
                            $"ApiAction {type} has more than one route constraint with the same key: {key}");
                    }

                    constraintDictionary.Add(key, routeConstraint);
                }

                var defaultDictionary = typeInfo.GetCustomAttributes<RouteDefaultAttribute>()
                    .ToDictionary(a => a.ParameterName, a => a.Value);

                foreach (var route in routes)
                {
                    services.AddSingleton(typeof(IApiActionRegistration),
                        new ApiActionRegistration(type, preparedRoutePrefix + route, constraintDictionary,
                            defaultDictionary));
                }
            }
        ...
        public static IServiceCollection AddApiRouteConstraint<T>(this IServiceCollection services)
            where T : IGlobalRouteConstraintApplicationFactory
        {
        ...
        public static IServiceCollection AddApiRouteConstraint(this IServiceCollection services,
            params IGlobalRouteConstraintApplicationFactory[] routeConstraintApplicationFactories)
        {
            if (services == null) throw new ArgumentNullException(nameof(services));
            if (routeConstraintApplicationFactories == null)
                throw new ArgumentNullException(nameof(routeConstraintApplicationFactories));
        ...
        public static IServiceCollection AddApiRouteConstraint<T>(this IServiceCollection services,
            ...
            where T : IRouteConstraint
        {
            return AddApiRouteConstraint(services,
                new GlobalRouteConstraintApplicationFactoryTypeWrapper(typeof(T), filters ?? new IActionTypeFilter[0]));
        }
            ...
            IRouteConstraint routeConstraint, params IActionTypeFilter[] filters)
        {
            ...
            return AddApiRouteConstraint(services,
                new GlobalRouteConstraintApplicationFactoryInstanceWrapper(routeConstraint,
                    filters ?? new IActionTypeFilter[0]));
        }
        ...
        public static IServiceCollection AddApiRouteDefaults(this IServiceCollection services,
            params IGlobalRouteDefaultApplicationFactory[] routeDefaultApplicatonFactories)
        {
            if (services == null) throw new ArgumentNullException(nameof(services));
            if (routeDefaultApplicatonFactories == null)
                throw new ArgumentNullException(nameof(routeDefaultApplicatonFactories));
        ...
        public static IServiceCollection AddApiRouteDefaults(this IServiceCollection services,
            IReadOnlyDictionary<string, object> defaults, string namespacePrefix)
        {
            if (namespacePrefix == null) throw new ArgumentNullException(nameof(namespacePrefix));
            ...
            IReadOnlyDictionary<string, object> defaults, params IActionTypeFilter[] filters)
        {
            ...
            return AddApiRouteDefaults(services,
                new GlobalRouteDefaultApplicationFactory(defaults, filters ?? new IActionTypeFilter[0]));
        }
        ...
        public static IServiceCollection AddApiAuthFilter<T>(this IServiceCollection services)
            where T : IGlobalAuthFilterApplicationFactory
        {
        ...
        public static IServiceCollection AddApiAuthFilter(this IServiceCollection services,
            params IGlobalAuthFilterApplicationFactory[] authFilterApplicationFactories)
        {
            if (services == null) throw new ArgumentNullException(nameof(services));
            if (authFilterApplicationFactories == null)
                throw new ArgumentNullException(nameof(authFilterApplicationFactories));
            ...
            where T : IAuthFilter
        {
            return AddApiAuthFilter(services,
                new GlobalAuthFilterApplicationFactoryTypeWrapper(typeof(T), filters ?? new IActionTypeFilter[0]));
        }
