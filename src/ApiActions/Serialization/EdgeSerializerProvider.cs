...
namespace ApiActions.Serialization
{
    public class EdgeSerializerProvider : IEdgeSerializerProvider
    {
        ...
        public EdgeSerializerProvider(IEnumerable<IEdgeSerializer> serializers,
            IOptions<ApiActionConfiguration> configurationAccessor)
        {
            if (serializers == null) throw new ArgumentNullException(nameof(serializers));
            _configurationAccessor =
                configurationAccessor ?? throw new ArgumentNullException(nameof(configurationAccessor));
        ...
        private IEdgeSerializer Get(IEnumerable<MediaTypeHeaderValue> acceptHeader)
        {
            ...
            {
                if (accepts.MatchesAllTypes &&
                    MediaTypeHeaderValue.TryParse(_configurationAccessor.Value.AcceptAllHeaderDefaultMediaType,
                        out defaultAccepts) && !defaultAccepts.MatchesAllTypes)
                {
            ...
            return MediaTypeHeaderValue.TryParse(_configurationAccessor.Value.AcceptsHeaderNotMatchedDefaultMediaType,
                out defaultAccepts)
                ? Get(new[] {defaultAccepts})
                : null;
        }
    }
}