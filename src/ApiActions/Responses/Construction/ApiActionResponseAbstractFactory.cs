...
namespace ApiActions.Responses.Construction
{
    public class ApiActionResponseAbstractFactory : IApiActionResponseAbstractFactory
    {
        ...
        public ApiActionResponseAbstractFactory(IEnumerable<IApiActionResponseFactory> responseFactories)
        {
            ...
            _responseFactories = (responseFactories ?? new IApiActionResponseFactory[0])
                .Concat(new[] {new DefaultResponseFactory()}).ToArray();
        }

        /// <summary>
        /// Identifies and provides an <see cref="T:ApiActions.IApiActionResponseFactory" /> that can create a response corresponding with the desired type of data.
        /// </summary>
        /// <param name="scopedType">Data type that should be considered when constructing a response.</param>
        /// <returns>
        /// An <see cref="T:ApiActions.IApiActionResponseFactory" />
        /// </returns>
