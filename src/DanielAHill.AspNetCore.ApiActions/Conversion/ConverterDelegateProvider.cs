...
using System.Linq;
using DanielAHill.AspNetCore.ApiActions.Conversion.Caching;

namespace DanielAHill.AspNetCore.ApiActions.Conversion
{
    internal class ConverterDelegateProvider : IConverterDelegateProvider
    {
        private readonly ITypeConverter[] _typeConverters;
        private readonly ConcurrentDictionary<Type, ConcurrentDictionary<Type, ICachedConverter>> _cachedConverters = new ConcurrentDictionary<Type, ConcurrentDictionary<Type, ICachedConverter>>();
        ...
        public ConverterDelegate Get(Type sourceType, Type destinationType)
        {
            ...
            var destinationCacheLevel = _cachedConverters.GetOrAdd(sourceType, c => new ConcurrentDictionary<Type, ICachedConverter>());
            var converter = destinationCacheLevel.GetOrAdd(destinationType, t => CreateConverter(sourceType, destinationType));
        ...
        private ICachedConverter CreateConverter(Type sourceType, Type destinationType)
        {
            ...
            return converter.CanConvertFrom(sourceType) ? new TypeDescriptorConvertFromCachedConverter(converter) : null;
        }
